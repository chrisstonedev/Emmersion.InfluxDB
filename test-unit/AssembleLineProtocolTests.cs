...
namespace EL.InfluxDB.UnitTests
{
    public class AssembleLineProtocolTests
    {
        ...
        {
            var timestamp = DateTimeOffset.UtcNow;
            var point = new InfluxPoint("test_measurement", new[] { new Field("field1", 1), new Field("field2", "foo") }, timestamp);
            var result = AssembleLineProtocol.Assemble(point);
        ...
        {
            var timestamp = DateTimeOffset.UtcNow;
            var point = new InfluxPoint("test_measurement", new[] { new Field("field1", new DateTime(2018, 12, 30)), new Field("field2", Guid.Empty), new Field("field3", new object()),  }, timestamp);

            var result = AssembleLineProtocol.Assemble(point);

            var expectedTimestamp = timestamp.ToUnixTimeNanoseconds().ToString();
            Assert.That(result, Is.EqualTo($@"test_measurement field1=""{new DateTime(2018, 12, 30)}"",field2=""{Guid.Empty}"",field3=""{new object()}"" {expectedTimestamp}"));
        }
        ...
        {
            var timestamp = DateTimeOffset.UtcNow;
            var point = new InfluxPoint("test measure,ment", new[] { new Field("field1", 1) }, null, timestamp);
            var result = AssembleLineProtocol.Assemble(point);
        ...
        public void should_use_default_timestamp_when_not_provided()
        {
            ...
            var point = new InfluxPoint("test_measurement", new[] { new Field("field1", 1) });
            var result = AssembleLineProtocol.Assemble(point);
